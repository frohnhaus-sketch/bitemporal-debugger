...
>
        ...
        <section style={{ display: "grid", gap: 24 }}>
          <WhiteCard
            eyebrow="Problem"
            title="Business entities change over time, but reports need the correct state at a specific date."
          >
            <p style={paragraphStyle}>
            ...
            <p style={paragraphStyle}>
              ...
              intervals so reports can answer what was true at a reporting date.
            </p>
            ...
            <ChipRow
              chips={[
                ...
                "Overlapping states",
                "Missing coverage",
                "Ambiguous point-in-time results",
              ]}
            />
          </WhiteCard>
          ...
          >
            ...
            <p style={paragraphStyle}>
              Once users ask “what was true on this date?”, the model needs
              explicit valid-time intervals.
            </p>
          ...
          <WhiteCard
            ...
            title="Store one row per entity state version."
          >
            <div style={solutionGridStyle}>
              <MiniCard
                title="Valid-time intervals"
                text="Use valid_from and valid_to to describe when each state version is true in the business."
              />
              <MiniCard
                title="One active state"
                text="Validate that each entity resolves to one intended state for a reporting date."
              />
              <MiniCard
                title="Boundary convention"
                text="Define interval semantics explicitly. Historical models should use one consistent interpretation of valid_from and valid_to across all entities and joins."
              />
              <MiniCard
                title="Correction tracking"
                text="Use bitemporal modeling when historical states can be corrected or delivered late."
              />
            </div>
          </WhiteCard>
          ...
          <WhiteCard
            ...
            title="Validate that point-in-time state resolution is unambiguous."
          >
            <CheckChipRow
              checks={[
                "Detect valid-time overlaps",
                "Detect valid-time gaps",
                "Validate current-state uniqueness",
                "Check point-in-time state resolution",
                "Validate interval boundary conventions",
              ]}
            />
          </WhiteCard>
          ...
          <WhiteCard
            ...
            title="State Modeling is the foundation for most historical reporting models."
          >
            <p style={paragraphStyle}>
  ...
  return (
    <section style={darkCardStyle}>
      ...
      <div style={exampleNoteStyle}>
        <div style={exampleNoteLabelStyle}>Point-in-time result</div>

        <p style={exampleNoteTextStyle}>
          A report for May should show the contract as Active. A report for
          August should show it as Cancelled.
        </p>
      </div>
    </section>
  );
...
function StateCard({
  ...
  return (
    <div style={stateCardStyle}>
      <div style={stateLabelStyle}>{report}</div>
      <div style={stateStatusStyle}>{status}</div>
      <div style={stateRangeStyle}>Valid: {range}</div>
    </div>
  );
}
  ...
  return (
    <section style={tryItCardStyle}>
      ...
      <p style={tryItTextStyle}>
        Use the Historical Modeling Workbench to reason about valid-time gaps,
        overlaps, point-in-time joins and historical state resolution.
      </p>
...
const stateStatusStyle: CSSProperties = {
  color: "#ffffff",
  fontSize: 20,
  fontWeight: 900,
  marginBottom: 6,
